...
void crt::copy_memory(void *const destination, const void *const source,
                      const std::uint64_t size) {
  __movsb(static_cast<std::uint8_t *>(destination),
          static_cast<const std::uint8_t *>(source), size);
}

void crt::set_memory(void *const destination, const std::uint8_t value,
                     const std::uint64_t size) {
  __stosb(static_cast<std::uint8_t *>(destination), value, size);
}

// Note: memset and __chkstk are implemented in asm_funcs.asm
// to avoid MSVC intrinsic conflicts
...
void crt::mutex_t::release() { _InterlockedExchange64(&value_, 0); }
...
void crt::bitmap_t::set_value(const pointer value) { value_ = value; }
