...
void logs::set_filter_cr3(std::uint64_t cr3) { filter_cr3 = cr3; }

void logs::clear_filter_cr3() { filter_cr3 = 0; }

std::uint64_t logs::get_filter_cr3() { return filter_cr3; }

std::uint8_t logs::flush(const cr3 slat_cr3,
                         const std::uint64_t guest_virtual_buffer,
                         const cr3 guest_cr3, const std::uint16_t count) {
  log_mutex.lock();
  ...
  const std::uint64_t bytes_written =
      memory_manager::operate_on_guest_virtual_memory(
          slat_cr3, &stored_logs[copy_start_index], guest_virtual_buffer,
          guest_cr3, write_size, memory_operation_t::write_operation);
