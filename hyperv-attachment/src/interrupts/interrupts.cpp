...
void set_up_nmi_handling() {
  ...
  const auto interrupt_gates =
      reinterpret_cast<segment_descriptor_interrupt_gate_64 *>(
          idtr.base_address);
  ...
  if (new_gate.present == 0) {
    ...
    original_nmi_handler = nmi_gate->offset_low |
                           (nmi_gate->offset_middle << 16) |
                           (static_cast<uint64_t>(nmi_gate->offset_high) << 32);
  }
...
void interrupts::set_up() {
  constexpr std::uint64_t processor_nmi_state_count =
      0x1000 / sizeof(crt::bitmap_t::size_type);

  processor_nmi_states.set_value(
      static_cast<crt::bitmap_t::pointer>(heap_manager::allocate_page()));
