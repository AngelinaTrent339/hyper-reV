...
std::uint8_t hook::set_up() {
  kernel_detour_holder_shadow_page_mapped = static_cast<std::uint8_t *>(
  ...
  std::uint64_t shadow_page_physical =
      hypercall::translate_guest_virtual_address(
          reinterpret_cast<std::uint64_t>(
              kernel_detour_holder_shadow_page_mapped),
          sys::current_cr3);
  ...
  kernel_detour_holder_physical_page =
      hypercall::translate_guest_virtual_address(kernel_detour_holder_base,
                                                 sys::current_cr3);
  ...
  // in case of a previously wrongfully ended session which would've left the
  // hook still applied
  hypercall::remove_slat_code_hook(kernel_detour_holder_physical_page);

  hypercall::read_guest_physical_memory(kernel_detour_holder_shadow_page_mapped,
                                        kernel_detour_holder_physical_page,
                                        0x1000);

  std::uint64_t hook_status = hypercall::add_slat_code_hook(
      kernel_detour_holder_physical_page, shadow_page_physical);

  if (hook_status == 0) {
    return 0;
  }

  kernel_detour_holder::set_up(
      reinterpret_cast<std::uint64_t>(kernel_detour_holder_shadow_page_mapped),
      0x1000);
...
load_original_bytes_into_shadow_page(
  ...
  return hook_disasm::get_routine_aligned_bytes(
      ...
      d_inline_hook_bytes_size + extra_asm_byte_count, routine_to_hook_virtual);
}
...
std::uint8_t set_up_inline_hook(
    ...
    std::uint64_t &overflow_original_page_physical_address) {
  ...
  *reinterpret_cast<std::uint32_t *>(&jmp_to_detour_bytes[1]) =
      parted_subroutine_to_jmp_to.u.low_part;
  *reinterpret_cast<std::uint32_t *>(&jmp_to_detour_bytes[9]) =
      parted_subroutine_to_jmp_to.u.high_part;

  std::vector<std::uint8_t> inline_hook_bytes = extra_assembled_bytes;

  inline_hook_bytes.insert(inline_hook_bytes.end(), jmp_to_detour_bytes.begin(),
                           jmp_to_detour_bytes.end());

  if (post_original_assembled_bytes.empty() == 0) {
    inline_hook_bytes.insert(inline_hook_bytes.end(),
                             post_original_assembled_bytes.begin(),
                             post_original_assembled_bytes.end());

    std::uint64_t nop_bytes_needed =
        original_bytes.second - inline_hook_bytes.size();

    inline_hook_bytes.insert(inline_hook_bytes.end(), nop_bytes_needed,
                             0x90); // nop padding until next actual instruction
  }
  ...
  if (is_overflow_hook == 1) {
    ...
    overflow_shadow_page_physical_address =
        hypercall::translate_guest_virtual_address(
            reinterpret_cast<std::uint64_t>(overflow_shadow_page_virtual),
            sys::current_cr3);
    ...
    const std::uint64_t overflow_page_virtual_address =
        routine_to_hook_virtual + 0x1000;
    ...
    const std::uint64_t prior_page_copy_size =
        inline_hook_bytes.size() - bytes_overflowed;

    memcpy(shadow_page_virtual + page_offset, inline_hook_bytes.data(),
           prior_page_copy_size);
    memcpy(overflow_shadow_page_virtual,
           inline_hook_bytes.data() + prior_page_copy_size, bytes_overflowed);
  } else {
    memcpy(shadow_page_virtual + page_offset, inline_hook_bytes.data(),
           inline_hook_bytes.size());
  }

  return 1;
}

std::uint8_t set_up_hook_handler(
    std::uint64_t routine_to_hook_virtual,
    std::uint16_t &detour_holder_shadow_offset,
    const std::pair<std::vector<std::uint8_t>, std::uint64_t> &original_bytes,
    const std::vector<uint8_t> &extra_assembled_bytes,
    const std::vector<uint8_t> &post_original_assembled_bytes) {
  ...
  *reinterpret_cast<std::uint32_t *>(&return_to_original_bytes[9]) =
  ...
  void *bytes_buffer = kernel_detour_holder::allocate_memory(
      static_cast<std::uint16_t>(hook_handler_bytes.size()));

  if (bytes_buffer == nullptr) {
    return 0;
  }

  detour_holder_shadow_offset =
      kernel_detour_holder::get_allocation_offset(bytes_buffer);
...
std::uint8_t hook::add_kernel_hook(
    std::uint64_t routine_to_hook_virtual,
    const std::vector<std::uint8_t> &extra_assembled_bytes,
    const std::vector<uint8_t> &post_original_assembled_bytes) {
  if (kernel_hook_list.contains(routine_to_hook_virtual) == true) {
    std::println("    [!] Hook already exists at 0x{:X}",
                 routine_to_hook_virtual);
    return 0;
  }

  // Validate the target address looks like a valid kernel function
  if ((routine_to_hook_virtual >> 48) != 0xFFFF) {
    std::println("    [!] Invalid kernel address 0x{:X} - not a canonical "
                 "kernel address",
                 routine_to_hook_virtual);
    return 0;
  }
  ...
  if (routine_to_hook_physical == 0) {
    std::println("    [!] Failed to translate VA 0x{:X} to PA - page not "
                 "present or CR3 invalid",
                 routine_to_hook_virtual);
    return 0;
  }

  // Additional validation: Read the target bytes and verify they look like
  // valid code
  std::uint8_t target_bytes[8] = {0};
  std::uint64_t bytes_read = hypercall::read_guest_virtual_memory(
      target_bytes, routine_to_hook_virtual, sys::current_cr3,
      sizeof(target_bytes));
  if (bytes_read != sizeof(target_bytes)) {
    std::println("    [!] Failed to read target memory at 0x{:X}",
                 routine_to_hook_virtual);
    return 0;
  }

  // Quick sanity check - target should not be all zeros or all 0xCC (int3
  // padding)
  bool all_same = true;
  for (int i = 1; i < 8; i++) {
    if (target_bytes[i] != target_bytes[0]) {
      all_same = false;
      break;
    }
  }
  if (all_same && (target_bytes[0] == 0x00 || target_bytes[0] == 0xCC)) {
    std::println("    [!] Target at 0x{:X} appears to be invalid (all {:02X})",
                 routine_to_hook_virtual, target_bytes[0]);
    return 0;
  }
  ...
  if (shadow_page_virtual == nullptr) {
    std::println("    [!] Failed to allocate shadow page memory");
    return 0;
  }
  ...
  if (shadow_page_physical == 0) {
    std::println("    [!] Failed to translate shadow page VA to PA");
    sys::user::free_memory(shadow_page_virtual);
    return 0;
  }
  ...
  if (original_bytes.first.empty() == true) {
    std::println("    [!] Failed to load original bytes from target "
                 "(disassembly failed?)");
    sys::user::free_memory(shadow_page_virtual);
    return 0;
  }
  ...
  std::uint8_t status = set_up_hook_handler(
      routine_to_hook_virtual, detour_holder_shadow_offset, original_bytes,
      extra_assembled_bytes, post_original_assembled_bytes);

  if (status == 0) {
    std::println("    [!] Failed to set up hook handler in detour holder");
    sys::user::free_memory(shadow_page_virtual);
    return 0;
  }

  std::uint64_t detour_address =
      kernel_detour_holder_base + detour_holder_shadow_offset;
  ...
  if (hook_status == 0) {
    std::println("    [!] Failed to set up inline hook");
    sys::user::free_memory(shadow_page_virtual);
    return 0;
  }

  hook_status = hypercall::add_slat_code_hook(routine_to_hook_physical,
                                              shadow_page_physical);

  if (hook_status == 0) {
    std::println("    [!] Failed to add SLAT hook via hypervisor (PA=0x{:X})",
                 routine_to_hook_physical);
    sys::user::free_memory(shadow_page_virtual);
    return 0;
  }
  ...
  if (overflow_shadow_page_physical_address != 0) {
    hook_status =
        hypercall::add_slat_code_hook(overflow_original_page_physical_address,
                                      overflow_shadow_page_physical_address);

    if (hook_status == 0) {
      std::println("    [!] Failed to add overflow SLAT hook");
      hypercall::remove_slat_code_hook(routine_to_hook_physical);
      sys::user::free_memory(shadow_page_virtual);
      return 0;
    }
  }
  ...
  hook_info.overflow_original_page_pfn =
      overflow_original_page_physical_address >> 12;
...
std::uint8_t hook::remove_kernel_hook(std::uint64_t hooked_routine_virtual,
  ...
  if (hypercall::remove_slat_code_hook(hook_info.original_page_pfn << 12) ==
      0) {
  ...
  if (hook_info.overflow_original_page_pfn != 0 &&
      hypercall::remove_slat_code_hook(hook_info.overflow_original_page_pfn
                                       << 12) == 0) {
  ...
  void *detour_holder_allocation =
      kernel_detour_holder::get_allocation_from_offset(
