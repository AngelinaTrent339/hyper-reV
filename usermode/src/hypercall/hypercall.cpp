...
std::uint64_t hypercall::enable_syscall_intercept(std::uint8_t mode) {
  hypercall_type_t call_type = hypercall_type_t::enable_syscall_intercept;

  return make_hypercall(call_type, 0, mode, 0, 0);
}

std::uint64_t hypercall::disable_syscall_intercept() {
  hypercall_type_t call_type = hypercall_type_t::disable_syscall_intercept;

  return make_hypercall(call_type, 0, 0, 0, 0);
}

std::uint64_t hypercall::set_syscall_filter(std::uint64_t syscall_min,
                                            std::uint64_t syscall_max,
                                            std::uint64_t cr3_filter) {
  hypercall_type_t call_type = hypercall_type_t::set_syscall_filter;

  return make_hypercall(call_type, 0, syscall_min, syscall_max, cr3_filter);
}

std::uint64_t
hypercall::flush_syscall_logs(std::vector<syscall_log_entry_t> &logs) {
  hypercall_type_t call_type = hypercall_type_t::flush_syscall_logs;
...
std::uint64_t hypercall::get_syscall_log_count() {
  hypercall_type_t call_type = hypercall_type_t::get_syscall_log_count;

  return make_hypercall(call_type, 0, 0, 0, 0);
}

std::uint64_t hypercall::hook_lstar(std::uint64_t lstar_va,
                                    std::uint64_t shadow_page_pa) {
  hypercall_type_t call_type = hypercall_type_t::hook_lstar;
